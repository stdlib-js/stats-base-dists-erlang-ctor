...
function Erlang() {
	...
	if ( arguments.length ) {
		...
		if ( !isPositiveInteger( k ) ) {
			throw new TypeError( format( '0z58y,HB', k ) );
		}
		if ( !isPositive( lambda ) ) {
			throw new TypeError( format( '0z58q,HC', lambda ) );
		}
	} else {
		k = 1.0;
		lambda = 1.0;
	...
	defineProperty( this, 'k', {
		...
		'set': function set( value ) {
			if ( !isPositiveInteger( value ) ) {
				throw new TypeError( format( '0z58n,HD', value ) );
			}
			k = value;
		}
	});
	defineProperty( this, 'lambda', {
		...
		'set': function set( value ) {
			if ( !isPositive( value ) ) {
				throw new TypeError( format( '0z58k,HE', value ) );
			}
			lambda = value;
		}
	});
